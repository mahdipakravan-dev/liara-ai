...
export function HistoryView({ deployment }) {
  ...
  const logs = hasError
    ? ["Using cache", "Installing dependencies", "Build failed: dependency installation timed out."]
    : ["Using cache", "Installing dependencies", "Building Next.js application", "Uploading image", "Deployment queued in build zone"];
  return (
    <div className="mx-auto max-w-[980px] px-5 py-10 lg:px-8 lg:py-16">
      <div className="flex items-center justify-between"><div><p className="text-xs text-slate-500">برنامه / assistance</p><h1 className="mt-2 text-xl font-bold">تاریخچه‌ی استقرار</h1></div><Badge variant={hasError ? "warning" : "outline"}>{hasError ? "خطا در استقرار" : "در حال دپلوی"}</Badge></div>
